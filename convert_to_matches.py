...
def is_english_sheet(sheet_title):
    return sheet_title in ENGLISH_SHEETS

def write_matches(filename, game_name, matches):
    start_time = time.mktime(time.strptime("2023-01-01", "%Y-%m-%d"))

    print("Writing to " + filename)

    with open(filename, "w") as f:
        f.write("game_name " + game_name + "\n")
...
english_rayter_file = "wordfeudligan_english.txt"
...
# Only parse sheets that are seasons or cups
    if (sheet.title.startswith("Säsong") or sheet.title.startswith("Cup")):
        ...
        if is_english_sheet(sheet.title):
            english_matches = current_matches + english_matches
        else:
...
write_matches(rayter_file, "Wordfeudligan", matches)
write_matches(english_rayter_file, "Wordfeudligan English", english_matches)
